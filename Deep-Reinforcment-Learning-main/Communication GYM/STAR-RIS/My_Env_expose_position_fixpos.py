...
class My_Env(gym.Env):
    def __init__(self):
        ...
        self.num_states = 2*self.M * self.K + 2*self.N * self.K + 2*self.M*self.N + 2*self.K
    ...
    def get_state(self):
        ...
        state = np.append(state, np.array(self.P_K_list[0]))
        state = np.append(state, np.array(self.P_K_list[1]))
        return state
    ...
    def step(self, action):
        ...
        # move = (action[3*self.N+2*self.M*self.K : 3*self.N+2*self.M*self.K+2] - 0.5) * 10
        # self.STAR_position = [move[0], move[1], self.STAR_position[2]]
